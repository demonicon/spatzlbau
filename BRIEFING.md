...
**Anfragen** (Auftrag 033): eine Aufgabe mit `type = anfrage`, verknüpft über `brief.anfrage.task_id` mit einer echten Aufgabe (Phase und Frist von dort). Stufen `briefing` (Entwurf) → `claude` (gesendet) → `ergebnis` → `entschieden`. Die App hält Anfragen in einer eigenen Liste (`state.anfragen`), deshalb erscheinen sie nie in Spalten, Timeline, Kennzahlen, Gate, Druck oder Suche; das Kalender-Abo filtert sie in der Edge Function heraus. Der Lauf schreibt `brief.vergleich` (`offers[]` mit `name`, `price`, `price_kind` fest/ab/monat, `service`, `term`, `valid_until`, `plus`, `minus`, `url`, `source`, `author`, `date`; dazu `recommendation`, optional `recommended` = id des empfohlenen Angebots, `reason`, `sources[]`, `request_text`) über `claude-result.mjs` (Feld `vergleich`) und setzt `claude → ergebnis`. Nie `chosen*`, nie `entschieden`, nie ein Angebot mit `author` S/A oder `source = manual` – der Merge in `app/anfragen.js` erzwingt das. „Wählen" in der App setzt `chosen`/`chosen_by`/`chosen_at`, `entschieden` und schreibt eine Entscheidung (032) an die verknüpfte Aufgabe; danach fragt die App, ob der Preis übernommen wird (Einmalpreis → Posten der Aufgabe, Monatspreis → `recurring`, vorerst nur Internet – Strom/Gas/Versicherung haben bis Auftrag 031 keine Laufend-Zeile).
...
1. **Kopf** (kompakt seit 013) – eine Zeile mit Avatar-Kürzel, vier Navigations-Pillen (Haus = Aufgaben, Münze = Finanzen, Waage = Anfragen seit 033, Raute = Entscheidungen seit 032b; aktiv mit Ink-Umriss, unter 900 px nur das Icon, Zähler-Badge in Ink bei offenen Ergebnissen bzw. Entscheidungen) und der Statuszeile; darunter der Countdown in einer Zeile („115 Tage bis zur Schlüsselübergabe · 15.01.2027", Zahl 28 px, das Datum öffnet das Datumsfeld) und die fünfteilige **Gate-Leiste** auf halber Höhe: Nummer und Füllbalken, der Zähler steckt im Tooltip.
...
Breiten (Stand 013): bis 599 px Handy (Akte inline unter der Aufgabe, die Aufgabenzeile ist dann der Kopf der Akte), 600–899 px Tablet (eine Spalte bis 760 px), **900–1179 px** Liste über die volle Breite und Akte als **Overlay von rechts** (440 px, abgedunkelt, schließt mit ×, Escape, Tippen daneben), **ab 1180 px** Liste und stilles Seitenpanel nebeneinander; der Container wächst bis 1800 px, die Spalten halten mindestens 320 px (`auto-fit`), das Panel liegt zwischen 420 und 600 px. Die offene Aufgabe steht in der URL (`#task=<id>`) und lässt sich als Link teilen – auf allen Breiten; `#finanzen` legt einen History-Eintrag an, eine geöffnete Akte nicht.
...
**Anfragen** (033): eigene Seite `#anfragen` (einzelne Anfrage `#anfragen=<id>`). Ab 900 px links die Liste (320 px, Ergebnisse zuerst, Vier-Stufen-Leiter, Chip „Ergebnis da" in Claude-Farbe), rechts die Anfrage; darunter die Liste und die Anfrage als eigene Seite. Entwurf: Formular „Rahmen" je Kategorie (Umzug, Internet, Strom und Gas, Versicherung, Sonstiges; vorbelegt aus den Stammdaten, soweit es sie gibt) und als einziger Primär „An Claude senden". Ergebnis: Empfehlung in Claude-Farbe, Vergleichstabelle mit bis zu fünf Angeboten als Spalten (günstigster Preis fett, abgelaufenes Angebot grau), unter 900 px je Angebot eine Karte; „Wählen" (Sekundär), „ändern", „+ Angebot", Begründung, Quellen, bei Umzug „Anfragetext kopieren", darunter Kommentare als Rückkanal an Claude. Die Akte der verknüpften Aufgabe zeigt den Block „Anfrage" mit Leiter und Link.
