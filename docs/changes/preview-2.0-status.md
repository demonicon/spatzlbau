...
**Migrationen 010–017 sind in der Live-Datenbank angewendet** (geprüft 23.09.2026, lesend: Tabellen,
Spalten, Check-Werte, Settings-Schlüssel; 017 am selben Tag mit 014 eingespielt). Die Hinweise „ohne
sie …“ je Zeile gelten nur noch für eine Neueinrichtung ohne `schema.sql`.
Freigaben nach `main`: 2.0 (`1bb388e`, Tag `v2.0`), 2.0.1 (`cabce7e`, Tag `v2.0.1`), 2.0.2 (014, Tag `v2.0.2`).
...
| 014 Bugfix 2.0.2 | `fix/014` | fertig, in `main` (Tag `v2.0.2`) | `017_b014_revoke.sql` – angewendet |

## Migrationen in dieser Reihenfolge einspielen (alle angewendet – Reihenfolge für eine Neueinrichtung)
...
8. `017_b014_revoke.sql` (014) – `log_task_change()` nicht mehr per RPC aufrufbar

Offen bleibt die Edge Function: `supabase functions deploy ics --no-verify-jwt --project-ref <ref>`
