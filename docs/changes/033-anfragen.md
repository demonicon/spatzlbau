...
- [x] Ein Primär je Ansicht (Anfragen: „An Claude senden" nur im Entwurf; im Ergebnis keiner) – **simuliert** gerendert, nicht eingeloggt
- [x] Tabelle bis 5 Angebote ohne horizontales Scrollen bei 1280; 380 px Karten – **simuliert** gerendert und vermessen, nicht eingeloggt
...
- **Doku außerhalb von 033:** In BRIEFING.md und `rules/db.md` auch die Laufzeiten aus 032b (stündlich → 8, 12, 15, 18, 22 Uhr) nachgezogen – für BRIEFING.md von Sebastian mit Frage B freigegeben, in der Regel dieselbe Tatsache. `brief.ergebnis` → `brief.result` in BRIEFING.md korrigiert: der Code schrieb immer `result`, die Doku war falsch. Die Kopfzeile in BRIEFING.md (Abschnitt 5, Punkt 1) beschreibt jetzt alle vier Pillen – mit der Anfragen-Pille ließ sie sich nur richtig beschreiben, wenn auch die Entscheidungen-Pille aus 032b (Raute, Icon-only < 900 px, Badge) drinsteht; das geht über „Anfragen" hinaus und steht deshalb hier zur Bestätigung.
- **Reviewer-Runde 2:** zusätzlich das *gewählte* Angebot im Merge geschützt – ein späterer Lauf mit derselben Angebots-id hätte sonst Preis und Name ändern können, und die Tabelle hätte etwas anderes gezeigt als Entscheidungs-Kommentar und übernommener Posten. Die zwei nur simuliert geprüften Kästchen oben tragen jetzt „simuliert".
...
## Nachtrag 038c (26.09.2026): `setup_fee`

`offers[]` bekommt ein weiteres optionales Feld: `setup_fee` (Zahl, Einmalbetrag neben dem laufenden Preis, z. B. eine Anschlussgebühr bei `price_kind = monat`). Die Vergleichstabelle zeigt dafür die Zeile „Einmalig" (fehlt das Feld → „–"). „Wählen" schreibt bei vorhandenem `setup_fee` zusätzlich zur `recurring`-Übernahme einen Posten `fest` in Höhe von `setup_fee` an die verknüpfte Aufgabe (derselbe Zielposten wie bei einem Einmalpreis: der `…-schaetzung`-Posten, sonst der einzige offene, sonst neu). Keine SQL-Migration – `brief` ist jsonb. Das Feld liefert der Claude-Lauf (Prompt-Anpassung durch Claude im Chat); Menschen tragen es im Formular „Angebot ändern"/„+ Angebot" ein.
