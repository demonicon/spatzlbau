...
function paymentsHTML() {
  ...
  return `<div class="fin-payments">
    <button class="pill" data-act="fin-filter" data-to="due7" aria-pressed="${ui.finFilter === 'due7'}">Fällig in 7 Tagen <span class="n">${due7}</span></button>
    <button class="pill ${late ? 'late' : ''}" data-act="fin-filter" data-to="late" aria-pressed="${ui.finFilter === 'late'}">Überfällig <span class="n">${late}</span></button>
  </div>`;
}
...
function bufferHTML() {
  ...
  if (!row) {
    return `<div class="fin-buffer">
      ...
      <button class="btn-secondary" data-act="buffer-add">Puffer anlegen</button>
    </div>`;
  }
  const open = ui.bufferEdit;
  return `<div class="fin-buffer" data-id="${row.id}">
    <button class="btn-text" data-act="buffer-edit" aria-expanded="${open}">Puffer (${bufferPct()} %)</button>
    <b>${eur(row.amount)}</b>
    ${
      open
        ? `<div class="cost-form">
            ...
            <div class="row">
              <button class="btn-secondary" data-act="buffer-save" data-ref="${row.id}">Speichern</button>
              <button class="btn-secondary" data-act="buffer-pct-apply" data-ref="${row.id}">Satz auf Summe anwenden (${eur(suggestedBuffer())})</button>
              <button class="btn-secondary" data-act="buffer-cancel">Abbrechen</button>
            </div>
          </div>`
        : ''
    }
...
function cashflowHTML() {
  ...
  return `<section class="fin-block">
    ...
    ${
      missing
        ? `<p class="fin-hint">Auszugstermine fehlen – <button class="btn-text" data-act="fin-settings">in den Einstellungen setzen</button>. Ohne sie lässt sich die Doppelmiete nicht berechnen.</p>`
        : ''
    }
...
function recurringHTML() {
  ...
  return `<section class="fin-block fin-recurring" id="fin-recurring">
    ...
    ${
      ui.recAdd
        ? `<div class="row"><label class="lbl">Posten<input type="text" data-input="rec-label" placeholder="z. B. Strom"></label><button class="btn-secondary" data-act="rec-add-save">Hinzufügen</button><button class="btn-secondary" data-act="rec-add-cancel">Abbrechen</button></div>`
...
export function finanzenView() {
  const filterRow = ui.finFilter
    ? `<div class="filter-row">
        <button class="pill on filter-chip" data-act="fin-filter-clear" aria-label="Filter entfernen">Filter: ${FIN_FILTERS[ui.finFilter].label}<span class="x" aria-hidden="true">×</span></button>
      </div>`
    : '';
  return (
    updateBarHTML() +
    ...
    `<footer class="foot"><button class="btn-text" data-act="screen" data-to="dashboard">← Aufgaben</button><span class="spacer"></span><button class="btn-text" data-act="fin-settings">Einstellungen</button></footer>` +
    `</div>`
  );
}
